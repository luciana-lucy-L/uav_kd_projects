#!/usr/bin/env python3
"""
train_traj_kd.py — TrajKD (Option B) Training

Input  : front camera image  (3, 224, 224)
Output : ctrl_body           (4,)     [vx_b, vy_b, vz_b, yaw_rate]   (deployed)
         traj_body_xyz       (K, 3)   [dx, dy, dz] per waypoint      (training-only aux)
Loss   : L = lambda_traj * L_traj + L_ctrl
...
def run_epoch(model, loader, ctrl_loss_fn, traj_loss_fn, lambda_traj,
    ...
    with torch.set_grad_enabled(train):
        for batch in loader:
            ...
            traj_full = batch['traj_body'].to(device)       # (B, K, 4)  [dx,dy,dz,dyaw=0]
            traj      = traj_full[:, :, :3]                 # (B, K, 3)  drop dyaw (always 0)

            ctrl_pred, traj_pred = model(img)

            ctrl_loss = ctrl_loss_fn(ctrl_pred, ctrl)
            traj_loss = traj_loss_fn(traj_pred, traj)
            ...
            total_ate       += traj_loss_fn.ate(traj_pred, traj).item() * bs
            n               += bs
...
def main():
    ...
    parser.add_argument('--config', default='configs/train/traj_kd.yaml')
    ...
    log(f"Model   : TrajKDPolicy  {n_params:.2f}M params  (K={mcfg['K']})")
    ...
    log(f"\n{'Epoch':>5}  {'Train':>10}  {'Val':>10}  {'ctrl':>8}  {'traj':>8}  "
        f"{'vx':>8}  {'vy':>8}  {'vz':>8}  {'yr':>8}  {'ATE':>8}  {'LR':>10}")
    log('─' * 116)

    for epoch in range(1, epochs + 1):
        t0 = time.time()

        train_loss, train_ctrl, train_traj, _, _ = run_epoch(
            model, train_loader, ctrl_loss_fn, traj_loss_fn, lambda_traj,
            optimizer, device, train=True,
        )
        val_loss, val_ctrl, val_traj, dim_mse, val_ate = run_epoch(
        ...
        log(f"{epoch:>5}  {train_loss:>10.6f}  {val_loss:>10.6f}  "
            ...
            f"{val_ate:>8.4f}  {lr_now:>10.2e}  ({time.time()-t0:.1f}s)")
