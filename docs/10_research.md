...
> 相关：文献对照见 `11_related_work.md`，架构实现见 `20_architecture.md`
...
经 2026-09 文献核查，三句全部不成立（详细证据见 `11_related_work.md`）：
...
**A × B 构成训练网格**（每格一次训练，共 15 个配置）；**C 是评估维度**（不增加训练次数）。
...
**三者很可能同时存在，权重未知。** R1 的诊断实验（`41_diagnostics.md`）用于测量三者的相对贡献，其结论直接决定数据采集协议。
